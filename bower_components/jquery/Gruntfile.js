module.exports = function( grunt ) {
	...
	grunt.initConfig({
		...
		testswarm: {
			tests: "ajax attributes callbacks core css1 data deferred dimensions effects event manipulation offset queue selector serialize support traversing Sizzle".split(" ")
		},
	...
	grunt.registerTask( "selector", "Build Sizzle-based selector module", function() {
		...
		/**
			...
			sizzle-jquery.js1 -> sizzle between "EXPOSE" blocks,
	...
	grunt.registerMultiTask(
		...
		function() {
			...
			//  *:*:-css1           all except css1 and dependents (explicit > implicit)
			//  *:*:-css1:+effects  same (excludes effects because explicit include is trumped by explicit exclude of dependency)
	...
	// Change the map file to point back to jquery.js1 instead of jquery.pre-min.js1.
	...
	grunt.registerMultiTask( "post-uglify", function() {
		...
		this.files.forEach(function( mapping ) {
			...
			grunt.file.write( mapFileName, grunt.file.read( mapping.dest )
				...
				.replace( /"dist\//g, "\"" )
				// Refer to the source jquery.js1, not the temporary jquery.pre-min.js1.
				.replace( /\.pre-min\./g, "." )
