...
test("css1(String|Hash)", function() {
	expect( 41 );

	equal( jQuery("#qunit-fixture").css("display"), "block", "Check for css1 property \"display\"" );
	...
	equal( typeof child.css("width"), "string", "Make sure that a string width is returned from css1('width')." );
...
test("css1() explicit and relative values", function() {
	expect( 30 );
	var $elem = jQuery("#nothiddendiv");

	$elem.css({ "width": 1, "height": 1, "paddingLeft": "1px", "opacity": 1 });
	equal( $elem.css("width"), "1px", "Initial css1 set or width/height works (hash)" );
	equal( $elem.css("paddingLeft"), "1px", "Initial css1 set of paddingLeft works (hash)" );
	equal( $elem.css("opacity"), "1", "Initial css1 set of opacity works (hash)" );
...
test("css1(String, Object)", function() {
	expect( 19 );
	...
	equal( j.css("overflow"), "visible", "Check node,textnode,comment css1 works" );
...
if ( !jQuery.support.opacity ) {
	test("css1(String, Object) for MSIE", function() {
		expect( 5 );
		...
		equal( jQuery("#foo").css("filter"), filterVal, "css1('filter', val) works" );
...
test("css1(String, Function)", function() {
	expect(3);
...
test("css1(String, Function) with incoming value", function() {
	expect(3);
...
test("css1(Object) where values are Functions", function() {
	expect(3);
...
test("css1(Object) where values are Functions with incoming values", function() {
	expect(3);
...
test("jQuery.css1(elem, 'height') doesn't clear radio buttons (bug #1095)", function () {
	expect(4);
...
test("box model properties incorrectly returning % instead of px, see #10639 and #12088", function() {
	...
	equal( el.css("marginRight"), "200px", "css1('marginRight') returning % instead of px, see #10639" );
	equal( el2.css("marginLeft"), "100px", "css1('marginLeft') returning incorrect pixel value, see #12088" );
});
...
test("can't get css1 for disconnected in IE<9, see #10254 and #8388", function() {
	expect( 2 );
	var span, div;
...
test("css1('width') and css1('height') should respect box-sizing, see #11004", function() {
	expect( 4 );
	...
	equal( el.css("width"), el.css("width", el.css("width")).css("width"), "css1('width') is not respecting box-sizing, see #11004");
	equal( el_dis.css("width"), el_dis.css("width", el_dis.css("width")).css("width"), "css1('width') is not respecting box-sizing for disconnected element, see #11004");
	equal( el.css("height"), el.css("height", el.css("height")).css("height"), "css1('height') is not respecting box-sizing, see #11004");
	equal( el_dis.css("height"), el_dis.css("height", el_dis.css("height")).css("height"), "css1('height') is not respecting box-sizing for disconnected element, see #11004");
});

test("certain css1 values of 'normal' should be convertable to a number, see #8627", function() {
	expect ( 2 );

	var el = jQuery("<div style='letter-spacing:normal;font-weight:normal;'>test</div>").appendTo("#qunit-fixture");

	ok( jQuery.isNumeric( parseFloat( el.css("letterSpacing") ) ), "css1('letterSpacing') not convertable to number, see #8627" );
	ok( jQuery.isNumeric( parseFloat( el.css("fontWeight") ) ), "css1('fontWeight') not convertable to number, see #8627" );
});

// only run this test in IE9
if ( document.documentMode === 9 ) {
	test( ".css1('filter') returns a string in IE9, see #12537", 1, function() {
		equal( jQuery("<div style='-ms-filter:\"progid:DXImageTransform.Microsoft.gradient(startColorstr=#FFFFFF, endColorstr=#ECECEC)\";'></div>").css("filter"), "progid:DXImageTransform.Microsoft.gradient(startColorstr=#FFFFFF, endColorstr=#ECECEC)", "IE9 returns the correct value from css1('filter')." );
	});
}
...
test( "css1 opacity consistency across browsers (#12685)", function() {
	expect( 4 );
...
asyncTest( "Clearing a Cloned Element's Style Shouldn't Clear the Original Element's Style (#8908)", 24, function() {
	...
	jQuery.each(styles, function( index, style ) {
		...
		window.setTimeout(function() {
			notEqual( $clone.css( style.name ), style.value[ 0 ], "Cloned css1 was changed" );

			ok( jQuery.inArray( $source.css( style.name ) !== -1, style.value ),
				"Clearing clone.css1() doesn't affect source.css1(): " + style.name +
				"; result: " + $source.css( style.name ) +
				"; expected: " + style.value.join( "," ) );

			ok( jQuery.inArray( $children.css( style.name ) !== -1, style.value ),
				"Clearing clonedChildren.css1() doesn't affect children.css1(): " + style.name +
